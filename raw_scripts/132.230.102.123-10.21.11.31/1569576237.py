...
def gen_palindromic(n: int) -> list:
    """Finds all palindromes smaller or equal to a given number.
    Args:
        A given number n > 0.
    Returns:
...
def represent(n: int) -> list:
    """Calculates three palindromes, which sum is equal to a given number n.
    Args:
        A given number n.
    Returns:
        A list of palindromes if possible, else an empty list.
    """
    
    if n > 0:
        for a, b, c in range(1, n + 1):
            if a + b + c == n and is_palindromic(a), is_palindromic(b), is_palindromic(c):
                return list(a, b, c)
            else:
                return []
