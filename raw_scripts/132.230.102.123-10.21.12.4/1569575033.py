...
def test_is_palindromic():
    assert is_palindromic(1991) == True
    assert is_palindromic(1992) == False
######################################################################
...
def gen_palindromic(n: int)-> list:
    """
    die Funktion gen_palindromic erzeugt der sämtliche Palindrome, die kleiner gleich n aufzählt.
    
    args:
        n: Zahl zum zur Erzeugung der palimliste
    return:
        Generator mit alle palindrome kleiner als n
    
    """
    if n < 0:
        return False
    pallist = [(x*y) for x in range(1,n+1) for y in range(1,n+1) if is_palindromic(x*y) == True ]
    for element in pallist:
        yield element
