...
def test_is_palindromic():
    assert not is_palindromic(37)
    assert is_palindromic(1)
    assert is_palindromic(1331)
    assert not is_palindromic(7766)
######################################################################
...
def gen_palindromic(n: int):
    """
    Generator for any palindrom smaller than n
    """
    assert n > 0
    for i in range(1, n):
        if is_palindromic(i):
...
def represent(n: int) -> list:
    """
    Determines the first up to 3 maximum palindrome numbers that
    sum up to n.
    """
    assert n > 0
    for i in range(1, n):
        if not is_palindromic(i):
            continue
        for j in range(1, n):
            if not is_palindromic(j):
                continue
            for k in range(1, n):
                if not is_palindromic(k):
                    continue
                
                # Test whether any given combination of sums of i, j and k can form the number n.
                if i + j == n:
                    return [i, j]
                if i + k == n:
                    return [i, k]
