...
def is_palindromic(n):
    ...
    teil_a = zahl_list[:((laenge+1)//2)]
    teil_b = zahl_list[((laenge+1)//2):]
    if teil_a == teil_b:
        return True
    else: 
        return False
...
## Lösung Teil 2. (Tests)
def test_is_palindromic_a():
    number = 12345
    assert is_palindromic(number) == False
def test_is_palindromic_b():
    number = 1234321
    assert is_palindromic(number) == True
