...
def test_is_palindromic():
    assert is_palindromic(8008) == True
    assert is_palindromic(800) == False
    assert is_palindromic(8) == True
...
def represent(n: int):
    ...
    for i in range(1, n):
        for j in range (1, n):
            for k in range (1,n):
                if str(i) == str(i)[-1::-1] and str(k) == str(k)[-1::-1] and str(j) == str(j[-1::-1]) and i + j + k == n:
                    return i, k , j
                else:
                    continue
