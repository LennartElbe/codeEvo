...
def test_is_palindromic():
    n1, n2, n3, n4, n5 = 1001, 2000, 20002, 34543, 5
    assert is_palindromic(n1) == True
    assert is_palindromic(n2) == False
    assert is_palindromic(n3) == True
    assert is_palindromic(n4) == True
    assert is_palindromic(n5) == True
######################################################################
...
def gen_palindromic(n):
    """A generator that yields all palindromic numbers < n.
    Args:
        n (int): upper boundary
    Returns:
        None (because its a generator function)
    """
    for i in range(1, n):
        if is_palindromic(i):
            yield i
