...
def test_palidromic():
    ...
    assert is_palindromic(3113) == True
    assert is_palindromic(3123) == False
    assert is_palindromic(313) == True
######################################################################
...
def gen_palindromic(n):
    """Generatorfunktion, die ganzzahligem Argument n>0 nimmt
    und die sämtliche Palindrome kleiner gleich n aufzählt.
    Args: n>0
    """
    for i in range(1,n):
        if is_palindromic(i):
            yield i
## Lösung Teil 4.
def represent(n:int) -> l:
    """ Funktion represent stellt eine natürliche Zahln n>0 als Summe von maximal drei Palindromen ungleich 0 dar.
    Die Funktion soll diese Palindrome als Liste zurückgeben, sonst die leere Liste.
    Args: n
    """
    l=[]
