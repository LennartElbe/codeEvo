...
def test_is_palindromic():
    assert(is_palindromic(999) == True)
    assert(is_palindromic(123321) == True)
    assert(is_palindromic(1000) == False)
    assert(is_palindromic(1) == True)
    assert(is_palindromic(-5) == "Error, integer has to be > 0")
    assert(is_palindromic(0) == "Error, integer has to be > 0")
######################################################################
...
def gen_palindromic(n: int):
    """Generator which calculates all palindromes lower than n
...
## Lösung Teil 4.
def represent(n: int):
    """Calculates the palindromes, which together forms the sum of n
    ...
    """
    # return [(x,y,z) for x in range(1, n + 1) for y in range(1, n + 1) for z in range (1, n + 1) if (x+y+z == n and str(x,y,z) == str(x,y,z)[::-1])] list_comprehension
    lst = []
    for i in range(1, n + 1):
        ...
        return []
    assert(represent(1) == [[1,0,0], [0,0,1], [0,1,0]]
######################################################################
## test code
