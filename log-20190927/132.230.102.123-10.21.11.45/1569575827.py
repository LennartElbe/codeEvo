...
## Lösung Teil 2. (Tests)
def test():
    """tested is_palindromic funktion"""
    assert(is_palindromic(1001))
    assert(is_palindromic(9595))
######################################################################
...
def gen_palindromic(n):
    """return a genrator from the palindrmoic numbers wich is smaller than n"""
    n > 0
    h = is_palindromic(n)
    while n >= h :
        yield h
...
## Lösung Teil 4.
def represent(n):
    """returns a sum from 3palindromic numbers
    """
    result = []
    h =gen_palindromic
    for i in h :
