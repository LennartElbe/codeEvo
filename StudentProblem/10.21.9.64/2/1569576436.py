...
def test_is_palindromic():
    """test of the function is_palindromic"""
    assert(is_palindromic(9009)) == True
    assert(is_palindromic(9109)) == False
######################################################################
## Lösung Teil 3.
def gen_palindromic(n: int) -> bool:
    """represent the generator Functon palindromic
    
    Args:
    n(int) = Natural Number
    
    Returns:the palindromic Number whos is less then n"""
    result = []
    if is_palindromic(n) == True:
...
def represent(n: int) -> list:
    if len(tuple(gen_palindromic(n))) < 3:
        return []
    else:
        result = []
        for i in tuple(genpalindromic(n)):
            result += [i]
        return resutl
######################################################################
## test code
