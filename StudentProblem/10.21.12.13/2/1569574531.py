...
def test_is_palindromic():
    ...
    assert is_palindromic(1002) == False
    assert is_palindromic(1231) == False
    assert is_palindromic(101) == True
######################################################################
## Lösung Teil 3.
def gen_palindromic(n: int) -> int:
    """Function to generate all palindromic numbers to a given range
    args: n: the range of our function
    """
...
def represent(n: int) -> list:
    """Funktion um zu prüfen ob gegebene Nummer als Palindorm dargestellt werden kann
    args: n : int nummer die geprüft wird
    """
    list_pal = []
    list_pal += [gen_palindromic(n)]
    if sum([list_pal]) == n:
        return list_pal
    else:
        return []
